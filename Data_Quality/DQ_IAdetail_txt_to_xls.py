...
#file variables
#readin_folder = 'C:/Temp/python/in/CTR DQ Results/MKTR_Wave1_QAT/Result/'
in_folder = 'C:/Temp/python/in/'
in_file = 'Copy of WAVE 2 Testing v3.xlsx'
readin_folder = 'C:/IBM/InformationServer11-5/ASBNode/bin/Result/'
out_folder = 'C:/Temp/python/out/'
out_file = 'report_DQ_IA_detail_fail_list_' + time.strftime("%Y%m%d") + '.xlsx'
...
if bGetUniqueId:
    ...
    DQ_Tests = pd.read_excel(in_folder + in_file ,'A. Expected Outcomes')
    DQ_Tests.drop([col for col in DQ_Tests.columns if col not in ['Table','Unique Record ID']],inplace=True,axis=1)
    DQ_Tests.drop_duplicates(inplace=True)
    table_list = [str(x).upper().strip() for x in DQ_Tests['Table'].tolist()]
    id_list = [str(x).upper().strip() for x in DQ_Tests['Unique Record ID'].tolist()]
    ...
    for filename in filenames:
        ...
        if '_detailresult.txt' in filename:
            ...
            if os.path.getsize(readin_folder + filename)>0:
                ...
                if this_tablename =="T_BILL_L": print filename 
...
#print CTR_filelist
print 'done, wrote sheetnames from ' + str(readin_folder) + ' to ' + str(out_folder) + str(out_file)
